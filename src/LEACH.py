...
class LEACHSimulation:
    ...
    def __create_sen(self):
        ...
        self.Sensors = LEACH_create_basics.create_sensors(self.my_model, Positions)

        # self.base_station_position = (101, 101)  # Set coordinates outside the field
        # self.Sensors[self.n].pos = self.base_station_position
    ...
    def __statistics(self, round_number):
        ...
        print("Sensors: ", )
        var_pp(self.Sensors)
        # self.alive_sensors = [100, 98, 95, 90, 85, 80, 75]
        print('----------------------------------------------')


        # Plotting the number of alive nodes over rounds
        # plt.plot(range(len(self.alive_sensors)), self.alive_sensors, marker='o', linestyle='-')
        # plt.title('Number of Alive Nodes Over Rounds')
        # plt.xlabel('Rounds')
